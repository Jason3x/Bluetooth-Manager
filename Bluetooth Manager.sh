...
GITHUB_URL="https://raw.githubusercontent.com/Jason3x/Bluetooth-Manager/main/patch.pak"
...
T_FINISH="Restart the system to apply the changes."
T_OGAGE="Stopping the ogage service..."
...
if [[ "$SYSTEM_LANG" == *"fr"* ]]; then
    ...
    T_FINISH="Redemarrer le systeme pour appliquer les changements."
T_OGAGE="Arret du service ogage..."
...
# --- ESPAÑOL (ES) ---
    ...
    T_FINISH="Reinicie el sistema para aplicar los cambios."
    T_OGAGE="Deteniendo el servicio ogage..."
...
# --- PORTUGUÊS (PT) ---
    ...
    T_FINISH="Reinicie o sistema para aplicar as alteracoes."
    T_OGAGE="Interrompe o serviço ogage..."
...
# --- ITALIANO (IT) ---
    ...
    T_FINISH="Riavviare il sistema per applicare le modifiche."
    T_OGAGE="Arresto del servizio ogage..."
...
# --- DEUTSCH (DE) ---
    ...
    T_FINISH="Starten Sie das System neu, um die Anderungen anzuwenden.."
    T_OGAGE="Der Ogage-Dienst wird beendet..."

# --- POLSKI (PL) ---
elif [[ "$SYSTEM_LANG" == *"pl"* ]]; then
    T_BACKTITLE="R36S Bluetooth Manager autorstwa Jasona"
    T_STARTING="Uruchamianie Bluetooth Manager...\nProsze czekac."
    T_CHECK_INTERNET="Sprawdzanie polaczenia z internetem..."
    T_INSTALL_DONE="Instalacja zakonczona!"
    T_INSTALL_TITLE="Instalacja"
    T_INSTALL_GAUGE="Trwa instalacja..."
    T_ERR_INTERNET="Internet jest wymagany do instalacji Bluetooth."
    T_ERR_TITLE="Blad"
    T_STOPPING="Zatrzymywanie Bluetooth..."
    T_STARTING_BT="Uruchamianie Bluetooth..."
    T_BT_DISABLED="Bluetooth jest wylaczony.\nWlacz go najpierw."
    T_SCAN_TITLE="Skanowanie"
    T_SCAN_GAUGE="Szukanie urzadzen..."
    T_NO_DEVICE="Nie wykryto zadnego urzadzenia."
    T_INFO="Informacja"
    T_NEARBY="Urzadzenia Bluetooth w poblizu"
    T_BACK="Powrot"
    T_CHOOSE_DEV="Wybierz urzadzenie:"
    T_SUCCESS="Sukces"
    T_CONNECTED="jest polaczony"
    T_RETRY="Pierwsza proba nieudana...\nRestartowanie Audio i ponawianie..."
    T_FAILED="Niepowodzenie"
    T_FAIL_CONNECT="Nie mozna polaczyc sie z"
    T_FAIL_MSG="Upewnij sie, ze urzadzenie jest w trybie parowania."
    T_FAIL_KNOWN="Sprawdz, czy urzadzenie jest WLACZONE i w zasiegu."
    ...
    T_DELETE_TITLE="Usun"
    T_CHOOSE_DEL="Wybierz urzadzenie do usuniecia:"
    T_DELETING="Usuwanie"
    T_FORGOTTEN="Urzadzenie usuniete."
    T_MAIN_TITLE="Menu Glowne"
    T_QUIT="Wyjdz"
    T_STATUS="Status Bluetooth"
    T_CONN_TO="Polaczony z"
    T_NONE="Brak"
    T_M_TOGGLE="Wlacz / Wylacz Bluetooth"
    T_M_SCAN="Skanuj i polacz"
    T_M_KNOWN="Znane urzadzenia"
    T_M_FORGET="Usun urzadzenie"
    T_M_QUIT="Wyjdz"
    T_FIXING_AUDIO="Naprawianie protokolow audio..."
    T_PAIRING="Trwa parowanie..."
    G_INIT="Inicjalizacja..."
    G_EXTRACT="Rozpakowywanie latki..."
    G_BIN="Instalowanie plikow binarnych..."
    G_SYSTEMD="Instalowanie uslug..."
    G_USER="Instalowanie plikow uzytkownika..."
    G_PKGS="Instalowanie pakietow..."
    G_BT="Wlaczanie Bluetooth..."
    G_AUDIO="Konfiguracja audio..."
    G_CLEAN="Czyszczenie..."
    G_ERROR="Blad instalacji"
    T_FINISH="Uruchom ponownie system, aby zastosować zmiany."
    T_OGAGE="Zatrzymywanie uslugi ogage..."
...
Installbt() {
    ...
    echo "--------------------------------------" > /dev/tty1
 echo "" > /dev/tty1
    echo "$T_OGAGE" > /dev/tty1
    systemctl stop oga_events
    
echo "--------------------------------------" > /dev/tty1
    echo "" > /dev/tty1

    echo "$G_INIT" > /dev/tty1
        mkdir -p /opt/system /tmp/patch
    
echo "--------------------------------------" > /dev/tty1 
echo "" > /dev/tty1    
    # Téléchargement et extraction
    echo "$G_EXTRACT" > /dev/tty1
        bash -c "wget -qO '${INSTALL_FOLDER}/patch.pak' '${GITHUB_URL}' && unzip -oq '${INSTALL_FOLDER}/patch.pak' -d '${INSTALL_FOLDER}/tmp' && rm -f '${INSTALL_FOLDER}/patch.pak'"
    echo "--------------------------------------" > /dev/tty1 
echo "" > /dev/tty1   
    ...
    echo "$G_BIN" > /dev/tty1
        bash -c '
            rm -rf /tmp/patch/*
            if [ -d "'"${INSTALL_FOLDER}"'/tmp/usr/local/bin" ]; then
                cp -r "'"${INSTALL_FOLDER}"'/tmp/usr/local/bin/"* /tmp/patch/ 2>/dev/null || true
            fi
            if [ "$(ls -A /tmp/patch/)" ]; then
                chmod 755 /tmp/patch/* && chown ark:ark /tmp/patch/* && mv -f /tmp/patch/* /usr/local/bin/
            fi
        '
    echo "--------------------------------------" > /dev/tty1 
echo "" > /dev/tty1   

    echo "$G_SYSTEMD" > /dev/tty1
        bash -c '
            rm -rf /tmp/patch/*
            if [ -d "'"${INSTALL_FOLDER}"'/tmp/etc/systemd/system" ]; then
                cp -r "'"${INSTALL_FOLDER}"'/tmp/etc/systemd/system/"* /tmp/patch/ 2>/dev/null || true
            fi
            if [ "$(ls -A /tmp/patch/)" ]; then
                chmod 644 /tmp/patch/* && chown root:root /tmp/patch/* && mv -f /tmp/patch/* /etc/systemd/system/
            fi
            systemctl daemon-reload
            [ -f /etc/systemd/system/watchforbtaudio.service ] && ln -sf /etc/systemd/system/watchforbtaudio.service /etc/systemd/system/multi-user.target.wants/ || true
            [ -f /etc/systemd/system/oga_events.service ] && ln -sf /etc/systemd/system/oga_events.service /etc/systemd/system/multi-user.target.wants/ || true
        '
    echo "--------------------------------------" > /dev/tty1 
echo "" > /dev/tty1   

    echo "$G_USER" > /dev/tty1
        bash -c '
            rm -rf /tmp/patch/*
            if [ -d "'"${INSTALL_FOLDER}"'/tmp/home/ark" ]; then
                cp -r "'"${INSTALL_FOLDER}"'/tmp/home/ark/"* /tmp/patch/ 2>/dev/null || true
            fi
            if [ "$(ls -A /tmp/patch/)" ]; then
                chmod 755 /tmp/patch/* && chown ark:ark /tmp/patch/* && mv -f /tmp/patch/* /home/ark/
            fi
        '
    echo "--------------------------------------" > /dev/tty1 
echo "" > /dev/tty1   

    echo "$G_PKGS" > /dev/tty1
        bash -c '
            if [ -f "'"${INSTALL_FOLDER}"'/tmp/packages.tar.gz" ]; then
                tar xzf "'"${INSTALL_FOLDER}"'/tmp/packages.tar.gz" -C "'"${INSTALL_FOLDER}"'/tmp"
                dpkg -i --force-overwrite "'"${INSTALL_FOLDER}"'/tmp/var/cache/apt/archives/"*.deb || true
            fi
        '
...
echo "" > /dev/tty1   
    ...
    echo "$G_AUDIO" > /dev/tty1
        bash -c '
            systemctl --global disable pulseaudio.service pulseaudio.socket || true
            usermod -aG bluetooth ark || true
            usermod -aG lp pulse || true
            usermod -aG lp,pulse-access ark || true
            if [ -d "'"${INSTALL_FOLDER}"'/tmp/etc/pulse" ]; then
                cp -r "'"${INSTALL_FOLDER}"'/tmp/etc/pulse/"* /etc/pulse/ 2>/dev/null || true
            fi
            systemctl enable pulseaudio || true
...
echo "" > /dev/tty1   
    ...
    echo "$G_CLEAN" > /dev/tty1
        bash -c "rm -Rf /tmp/patch '${INSTALL_FOLDER}/tmp' && echo 1 > /opt/system/bt.txt"
...
echo "$T_INSTALL_DONE" > /dev/tty1
    dialog --backtitle "$BACKTITLE" --title "$T_INSTALL_TITLE" --msgbox "\n$T_FINISH" 8 50 > "$CURR_TTY"
    shutdown now
}

# --- Dependances ---
CheckDeps() {
    # Si le fichier n'existe pas, on lance l'installation
...
ExitMenu() {
    ...
    systemctl start oga_events > /dev/null 2>&1
